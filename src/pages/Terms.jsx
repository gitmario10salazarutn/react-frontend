...
function Terms(props) {
    ...
    const openEditDialog = (id) => {
        getEducationById(id).then((resp) => {
            ...
            ToastCustom("warning", "Language " + resp.data.token.major + " found to edit!", "Successful!");
        });
        setEditDialog(true);
    };
    ...
    const handleSubmitEditEducation = (e) => {
        ...
        if (validate.length === 0) {
            if (data.password === data.rep_password) {
                axios
                    .put(url + "users/update_education/", data)
                    .then((res) => {
                        console.log(res);
                        let response = res.data;
                        if (response.token != null) {
    ...
    const confirmDeleteDialog = (id) => {
        getEducationById(id).then((resp) => {
            setEducation(resp.data.token);
            editData(resp.data.token);
            ToastCustom("warning", "Education " + resp.data.token.major + " found to remove!", "Successful!");
        });
    ...
    const deleteEducation = (e) => {
        e.preventDefault();
        axios
            .delete(url + "users/delete_education/" + data.id_education)
            .then((res) => {
                let response = res.data;
                if (response.token === 1) {
                    setDeleteDialog(false);
                    ...
                    <div className="confirmation-content">
                        ...
                        {education && (
                            <div className="question" style={{ fontSize: "1.1rem" }}>
                                Are you sure you want to delete education <b>{education.major}</b>?
                            </div>
                        )}
                    </div>
                </Dialog>
