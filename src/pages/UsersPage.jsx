...
const UsersPage = () => {
    ...
    const openEditUser = (id) => {
        getUser(id).then((resp) => {
            ...
            ToastCustom("warning", "User " + usermodel.user.email + " found to edit!", "Successful!");
        });
        setEditDialog(true);
    };
    ...
    const confirmDeleteUser = (id) => {
        getUser(id).then((resp) => {
            setUser(resp.data.token);
            usermodel.user = resp.data.token.user;
            ToastCustom("warning", "User " + usermodel.user.email + " found to remove!", "Successful!");
        });
    ...
    const deleteUser = (e) => {
        ...
        if (validate.length === 0) {
            axios
                .delete(url + "users/delete_user/" + user.user.user_name, {
                    data: { password: password },
                })
                .then((res) => {
                    let response = res.data;
                    if (response.token === 1) {
                        setDeleteUserDialog(false);
                        clearData();
                        ToastCustom("success", "User Deleted: " + user.user.email, "Successful!");
                    } else {
    ...
    const confirmsetEnableUser = (id) => {
        getUser(id).then((resp) => {
            setUser(resp.data.token);
            usermodel.user = resp.data.token.user;
            ToastCustom("warning", "User " + usermodel.user.email + " found to change state!", "Successful");
        });
    ...
    const setEnableUser = (e) => {
        ...
        axios
            ...
            .then((res) => {
                let response = res.data;
                if (response.token === 1) {
                    ToastCustom("success", "User state change to: " + user.user.email, "Successful!");
                } else {
                        ...
                        <div className="text-muted mb-0">
                            <div className="address">
                                <div className="icon-address">
                                    ...
                                    <p className="text-muted mb-0 sub-column">{rowData.user.email}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        if (rowData != null) {
            return (
                <div className="rol-user">
                    <p className="fw-normal mb-1 p-text">{rowData.user.rol_user.rol_name}</p>
                    ...
                    <div className="column-actions">
                        ...
                        <div className="icon-u">
                            <i
                                ...
                                title={rowData.user.user_state ? "Set disable user" : "Set enable user"}
        ...
        <div className="datatable-crud-demo container-table">
            ...
            <div className="card">
                ...
                >
                    <Column
                        ...
                        className="p-text s"
                    ></Column>
                    <Column header="Title" sortable body={userRol} className="p-text s"></Column>
                    <Column header="Status" body={userEnable} className="p-text s"></Column>
                    <Column header="Address" sortable body={userAddress} className="p-text s"></Column>
                    <Column
                        ...
                        className="p-text s"
                    ></Column>
                    <Column header="Actions" body={userActions} className="p-text s"></Column>
                </DataTable>
            </div>
            <Dialog
                visible={userDialog}
            ...
            >
                <div className="confirmation-content">
                    ...
                    {user && (
                        <div className="question" style={{ fontSize: "1.1rem" }}>
                            Are you sure you want to {user.user.user_state ? "disable" : "enable"} to:{" "}
                            <b>{user.user.email}</b>?
                        </div>
                    )}
                </div>
            ...
            >
                <div className="confirmation-content">
                    <img src="https://bit.ly/3lRKeeD" alt="" className="rounded-circle warn img-user" />
                    {user && (
                        <>
